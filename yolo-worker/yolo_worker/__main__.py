"""
Entrypoint: `python -m yolo_worker --app <app>`.

Config comes from the environment (set by the core plugin's worker.spawn); the
only CLI arg is `--app`, purely so the process line is self-describing in `ps`
and the plugin logs. A SIGTERM (how the core worker-hook stops workers) is
turned into a clean shutdown.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="yolo_worker")
    parser.add_argument("--app", default=None, help="App slug (overrides YOLO_APP).")
